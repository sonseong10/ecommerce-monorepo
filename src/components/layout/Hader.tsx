...
const GlobalHeader = ({ userId, userCard, toggleOverlay, toggleOpenSideBar, dark }: IGlobalHeaderProps) => {
    ...
    <header className={`sm-only ${styles.header} ${dark && styles.isDark}`}>
      <div className={`${styles.globalHeader} ${hide && styles.hide}`}>
        <strong className={styles.logo}>
          <Link to={userId ? '/' : '#'}>
            <img className="logo-img" src={!dark ? Logo : ''} alt="WhoMember" />
          </Link>
        </strong>
        ...
        >
          {!userId ? (
            ...
            <figure>
              <img className={styles.smProfile} src={userCard ? userCard!.fileURL : DEFAULT_USER_IMG} alt="" />
