...
const generateInsTag = (data?: { type: 'getshop' | 'viewus' | 'car' | 'ad'; code: string | number }) => {
  if (data === undefined) {
    return
  }
...
const generateScriptTag = () => {
  switch (process.env.REACT_APP_UI_ENV) {
    ...
    case 'test':
      return `<script src="https://in-ad-scdev.fastview.co.kr/integrated-ad.js"></script>`
    default:
      return `<script src="https://script.fastview.co.kr/integrated-ad.js"></script>`
  }
}
    ...
    <ButtonGroupWrapper>
      <Button
        ...
        onClick={() => {
          ...
          temp.value = generateInsTag(popupDo.data) + `\n` + generateScriptTag()
