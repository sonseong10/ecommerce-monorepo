...
export const useGridCheckBox = (id: string, init?: boolean, setValue?: (value: boolean) => void) => {
  const { checkValue } = useSelectorEq((state: ICommonsStore) => ({
    checkValue: state.ui.gridCheckbox !== undefined ? state.ui.gridCheckbox[id] : undefined,
  }))
...
export const useGridCheckBoxValue = () => {
  const { totalValue } = useSelectorEq((state: ICommonsStore) => ({
    totalValue: state.ui.gridCheckbox !== undefined ? state.ui.gridCheckbox : undefined,
  }))
