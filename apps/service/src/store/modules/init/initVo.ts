...
export function checkUserAgent() {
  if (getSession(SessionName.DEVICE) === null || getSession(SessionName.DEVICE) === undefined) {
...
export function checkFakePcAgent() {
  const isInApp = window.navigator.userAgent.toLocaleLowerCase().indexOf('smbridge');
  const isMobile = /mobi/i.test(window.navigator.userAgent);
